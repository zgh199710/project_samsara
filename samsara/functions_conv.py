...
from samsara.core import Function, as_variable
from samsara.utils import pair, get_conv_outsize, get_deconv_outsize
from samsara.functions import linear, broadcast_to
...
def conv2d_simple(x, W, b=None, stride=1, pad=0):
    ...
    参数:
        x (Variable): 输入数据，形状 (N, C, H, W)
        W (Variable): 卷积核权重，形状 (OC, C, KH, KW)
        b (Variable): 偏置，形状 (OC,)
        stride (int): 步长
        pad (int): 填充大小

    返回:
        Variable: 卷积结果，形状 (N, OC, OH, OW)
    """
...
def pooling_simple(x, kernel_size, stride=1, pad=0):
    """简化的池化操作。
        ...
        数学公式：
        y[n,c,i,j] = max_{0≤h<KH, 0≤w<KW} x[n,c,i*s+h,j*s+w]

        参数:
           x (Variable): 输入数据，形状 (N, C, H, W)
           kernel_size (int or tuple): 池化核大小
           stride (int): 步长
           pad (int): 填充大小

        返回:
           Variable: 池化结果，形状 (N, C, OH, OW)
        """
    x = as_variable(x)
...
class Conv2d(Function):
    ...
    def forward(self, x, W, b):
        """
           ...
           参数:
               x (ndarray): 输入数据，形状 (N, C, H, W)
               W (ndarray): 卷积核权重，形状 (OC, C, KH, KW)
               b (ndarray): 偏置，形状 (OC,)

           返回:
               ndarray: 卷积结果，形状 (N, OC, OH, OW)
           """
        xp = cuda.get_array_module(x)

        KH, KW = W.shape[2:]
        # print("Conv2d function input shape:", x.shape)
        # print("Conv2d function weight shape:", W.shape)
        col = im2col_array(x, (KH, KW), self.stride, self.pad, to_matrix=False)
        # print("im2col output shape:", col.shape)
...
class Deconv2d(Function):
    ...
    数学公式：
    x[n,c,i,j] = Σ_f Σ_h Σ_w y[n,f,(i-h)/s,(j-w)/s] * W[f,c,h,w]
...
def im2col(x, kernel_size, stride=1, pad=0, to_matrix=True):
    ...
    Args:
        x (Variable or ndarray): 输入变量，形状 (N, C, H, W)
        kernel_size (int or (int, int)): 卷积核大小
        stride (int or (int, int)): 步长
        pad (int or (int, int)): 填充大小
        to_matrix (bool): 如果为 True，将结果重塑为 2D 数组


    Returns:
       Variable: 输出变量。如果 to_matrix 为 False，输出形状为 (N, C, KH, KW, OH, OW)，
                  否则为 (N*OH*OW, C*KH*KW)。
...
def im2col_array(img, kernel_size, stride, pad, to_matrix=True):
    """使用 NumPy 实现的 im2col 操作。

    Args:
        img (ndarray): 输入图像，形状 (N, C, H, W)
        kernel_size (int or tuple): 卷积核大小
        stride (int or tuple): 步长
        pad (int or tuple): 填充大小
        to_matrix (bool): 是否将结果转换为矩阵

    Returns:
        ndarray: 转换后的列
    """
    # print("im2col input shape:", img.shape)
