...
import numpy as np
from samsara import as_variable
...
def get_dot_graph(output, verbose=True):
    ...
    def add_variable(v):
        nonlocal txt
        if v is not None:
            v = as_variable(v)  # Convert to Variable if it's not already
            if id(v) not in seen_set:
                txt += _dot_var(v, verbose)
                seen_set.add(id(v))
                if hasattr(v, 'creator') and v.creator is not None:
                    add_func(v.creator)
...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    tmp_dir = os.path.join(os.path.expanduser('~'), '.samsara')
...
def reshape_sum_backward(gy, x_shape, axis, keepdims):
    """Reshape gradient appropriately for samsara.functions.sum's backward.
...
def array_equal(a, b):
    ...
    Args:
        a, b (numpy.ndarray or cupy.ndarray or samsara.Variable): input arrays
            to compare
...
cache_dir = os.path.join(os.path.expanduser('~'), '.samsara')


def get_local_file(file_name=None):
